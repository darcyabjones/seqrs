...
impl<K, V, T> CodonTagTable<Gapped<K>, Gapped<V>> for T
    where T: CodonTagTable<K, V>
{
    fn get_tag(&self, k: &Gapped<K>) -> Gapped<V> {
        k.as_ref().map(|c| self.get_tag(c))
    }
}


pub trait CodonTags<O, T>: IntoIterator {
    fn codon_tags(self, table: T) -> CodonTagIterator<Self::IntoIter, T, O>;
}


impl<O, T, I> CodonTags<O, T> for I
...
#[derive(Debug, Clone)]
pub struct CodonTagIterator<I, T, O> {
    iter: I,
    table: T,
    b: PhantomData<O>,
}
...
impl<O, I, T> Iterator for CodonTagIterator<I, T, O>
    where I: Iterator,
...
impl<O, I, T> DoubleEndedIterator for CodonTagIterator<I, T, O>
...
impl<O, I, T> ExactSizeIterator for CodonTagIterator<I, T, O>
